...
supernova::headers::read_return supernova::headers::read_file(char const * filename) {
    ...
    if (!file) {
        return read_return{ .status = read_status::FileNotFound };
    }
    ...
    if (size < sizeof(main_header)) {
        return read_return{ .status = read_status::InvalidHeader };
    }

    if (main.magic != headers::master_magic) {
        return read_return{ .status = read_status::MagicMismatch };
    }
    ...
    if ((main.version | patch_mask) < (snvm_version | patch_mask)) {
        return read_return{ .status = read_status::VersionMismatch };
    }

    if (main.entry_point > main.memory_size) {
        return read_return{ .status = read_status::InvalidEntryPoint };
    }
    ...
    for (size_t i = 0; i < main.memory_regions; ++i)
    {
        ...
        if (region.offset + region.size > main.memory_size) {
            return read_return{ .status = read_status::InvalidMemoryRegion };
        }
    ...
    auto memory = std::make_unique_for_overwrite<uint8_t[]>(main.memory_size);
    ...
    return read_return{
        .memory_pointer = std::move(memory),
